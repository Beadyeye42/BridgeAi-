  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (referral && ["APPROVED", "REJECTED"].includes(parsed.data.status)) {
        ...
        await tx.affiliateNotification.create({ data: { affiliateId: referral.affiliateId, type: "SYSTEM", title: referralStatus === "APPROVED" ? "Referred supplier approved" : "Referred supplier rejected", body: `${existing.legalName} has been ${referralStatus.toLowerCase()} by Bridge-iT.`, actionUrl: "/affiliate/referrals" } });
