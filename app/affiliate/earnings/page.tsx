...
export default async function AffiliateEarningsPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string; supplier?: string; plan?: string; status?: string }> }) {
  ...
  const entries = await prisma.affiliateCommission.findMany({
    where: {
    ...
    include: { supplierCompany: { select: { legalName: true } }, membershipPlan: { select: { name: true } }, payoutItem: { include: { payout: { select: { paidAt: true, statementReference: true } } } } },
    orderBy: { earnedAt: "desc" },
  });
  const suppliers = await prisma.affiliateReferral.findMany({ where: { affiliateId: affiliate.id }, include: { supplierCompany: { select: { legalName: true } } }, orderBy: { supplierCompany: { legalName: "asc" } } });
  const plans = await prisma.membershipPlan.findMany({ where: { affiliateCommissions: { some: { affiliateId: affiliate.id } } }, select: { id: true, name: true }, orderBy: { monthlyPricePence: "asc" } });
