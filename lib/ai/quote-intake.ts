  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      instructions: [
        ...
        "The supplied category list contains only industries and products currently launched by Bridge AI. Choose the most specific matching child category silently. Never invent or return an unavailable category. Keep a specific product authoritative only while the customer is continuing the same job. If the newest message clearly introduces an incompatible product or industry, treat that as a conversation pivot rather than forcing it into an old field; the application will keep separate jobs from becoming mixed.",
