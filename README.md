...
- Supplier billing uses Stripe Checkout and Stripe Tax: £29.99 plus VAT per month for the first six billing months, then £49.99 plus VAT per month. Stripe Subscription Schedules enforce the price change. There are no introduction or winning fees. A customer selection transaction immediately creates the tenant-scoped contact-access grant, closes the request and audits both the selection and contact release.
...
For billing, create two recurring monthly GBP Stripe Prices, both excluding VAT: £29.99 and £49.99. Set their IDs as `STRIPE_INTRODUCTORY_PRICE_ID` and `STRIPE_STANDARD_PRICE_ID`, then set `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET`. Enable Stripe Tax for the account and register `https://<portal-host>/api/webhooks/stripe` for Checkout Session completion and customer subscription events. The verified webhook attaches a six-month introductory phase followed by the standard recurring phase. No success-fee product or checkout route is used.
