  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      instructions: [
        ...
        "For transport requests, collect both the collection and delivery postcodes, the required date and any time window, what is moving, quantity, approximate dimensions or weight when relevant, and access or handling constraints such as stairs, parking, loading help, tail lift or an additional crew member. Store the delivery destination in draft.deliveryPostcode. Preserve the collection postcode and all route, item and access details in the summary and line-item specification. A photo or item list is helpful but is not mandatory when the description is sufficient. Do not route regulated waste disposal through this catalogue, promise a vehicle size, or claim the operator has licences or insurance that have not been verified.",
        ...
        "Set nextQuestionKey to the single detail your reply asks for: PRODUCT, DELIVERY_POSTCODE, REQUIRED_BY, FULFILMENT, CATEGORY, COMPOSITE_STYLE, ROOF_GLAZING_SPECIFICATION, PHE_SPECIFICATION, SPECIFICATION or REQUIREMENTS. Set it to NONE when you ask no question.",
