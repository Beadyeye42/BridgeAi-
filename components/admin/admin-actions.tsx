...
export function RunProductionMonitoringButton(){const router=useRouter();const[busy,setBusy]=useState(false);const[message,setMessage]=useState("");return <div className="status-control"><button className="button button-dark" disabled={busy} onClick={async()=>{setBusy(true);setMessage("");try{const result=await call("/api/admin/system/monitor","POST",{});const recovered=Number(result.processedWhatsAppJobs??0);setMessage(result.configured?`${recovered} WhatsApp action${recovered===1?"":"s"} processed; ${result.sent} alert${result.sent===1?"":"s"} sent; ${result.queued} newly queued.`:`${recovered} WhatsApp action${recovered===1?"":"s"} processed. Alerts are queued, but email delivery needs configuration.`);router.refresh()}catch(error){setMessage(error instanceof Error?error.message:"Monitoring check failed")}finally{setBusy(false)}}}>{busy?<LoaderCircle className="spin" size={14}/>:<Send size={14}/>}Run monitoring check</button>{message&&<small className="form-result">{message}</small>}</div>}
