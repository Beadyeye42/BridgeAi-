...
import { AffiliatePayoutControls, AffiliateStatusControl } from "@/components/admin/affiliate-manager";
...
export default async function AdminAffiliateDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><AdminHeading eyebrow={affiliate.code} title={affiliate.displayName} description={`${affiliate.user.email} · permanent attribution and accounting history`} actions={<AffiliateStatusControl id={affiliate.id} status={affiliate.status} />} />
