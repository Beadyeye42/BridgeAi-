    ...
    <div className="management-grid">
      ...
      <section className="panel form-section">
        ...
        <div className="entity-list">{request.matchDecisions.length ? request.matchDecisions.map((decision) => {
          ...
          return <article className="entity-row" key={decision.id}><div><b>{decision.supplierCompany.tradingName ?? decision.supplierCompany.legalName}</b><small>{[decision.membershipTier, decision.coveragePurpose ? `${decision.coveragePurpose.toLowerCase()} coverage` : null, decision.distanceMiles ? `${Number(decision.distanceMiles).toFixed(1)} miles` : null].filter(Boolean).join(" · ")}</small><small>{reasons.join(" · ") || "No explanation recorded"}</small></div><span className={`status-pill ${decision.selected ? "accepted" : decision.outcome.toLowerCase()}`}>{decision.selected ? `Selected · ${decision.score}` : `${decision.outcome} · ${decision.score}`}</span></article>;
