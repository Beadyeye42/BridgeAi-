...
import { AffiliateInvitationControl, AffiliatePayoutControls, AffiliateProfileControl, AffiliateStatusControl } from "@/components/admin/affiliate-manager";
...
export default async function AdminAffiliateDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [affiliate, programme] = await prisma.$transaction([prisma.affiliate.findUnique({ where: { id }, include: {
    user: true,
  ...
  } }), prisma.affiliateProgramme.findUniqueOrThrow({ where: { id: "default" } })]);
  ...
  return <><AdminHeading eyebrow={affiliate.code} title={affiliate.displayName} description={`${affiliate.user.email} · permanent attribution and accounting history`} actions={<><AffiliateInvitationControl id={affiliate.id} /><AffiliateStatusControl id={affiliate.id} status={affiliate.status} /></>} />
    ...
    <section className="panel"><p className="eyebrow">Payout controls</p><h2>Statements</h2><p>Only commission that has passed the {programme.validationDays}-day validation period is included.</p><AffiliatePayoutControls affiliateId={affiliate.id} scheduled={affiliate.payouts.filter((payout) => payout.status === "SCHEDULED").map((payout) => ({ id: payout.id, reference: payout.statementReference }))} /></section>
