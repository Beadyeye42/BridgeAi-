...
describe("security foundation static controls", () => {
  ...
  it("uses the shared request deadline and selects no more than five suppliers", () => {
    ...
    const capabilityRematcher = read("lib/matching/rematch.ts");
    ...
    expect(capabilityRematcher).toMatch(/expiresAt:\s*quote\.responseDueAt/);
    expect(capabilityRematcher).not.toContain("invitationExpiresAt");
