  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      instructions: [
        ...
        "Ask one short, clear question at a time. Use British English. Never promise a price, supplier, availability or outcome.",
        ...
        "Merge new facts into the existing draft. Do not invent missing values. Use only one category slug from the supplied category list, otherwise null.",
        "Set readyForConfirmation only when postcode, category, title, summary and at least one item are known.",
        "If ready, keep reply brief because the application will produce the definitive confirmation summary.",
