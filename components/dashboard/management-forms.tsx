...
export function CompanyProfileForm({ company, categories, selectedCategoryIds }: { company: Record<string, unknown>; categories: { id: string; name: string; description: string | null; groupName: string }[]; selectedCategoryIds: string[] }) {
  ...
  return <form className="management-form" onSubmit={submit}>
    ...
    <section className="panel form-section" id="product-categories"><div className="section-heading"><div><p className="eyebrow">Matching</p><h2>Products you supply</h2></div></div><p className="body-copy">Select only products your company actively supplies and can quote. Bridge AI uses these exact choices to match enquiries to your business, so do not select products you cannot fulfil.</p>{Array.from(new Set(categories.map((category) => category.groupName))).map((groupName) => <div className="category-group" key={groupName}><h3>{groupName}</h3><div className="choice-grid">{categories.filter((category) => category.groupName === groupName).map((category) => <label className="choice-card" key={category.id}><input type="checkbox" name="categoryIds" value={category.id} defaultChecked={selectedCategoryIds.includes(category.id)} /><span><b>{category.name}</b><small>{category.description}</small></span></label>)}</div></div>)}</section>
