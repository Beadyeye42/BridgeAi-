...
export default async function MembershipAdminPage() {
  ...
  const coverageGaps = await prisma.coverageGapSignal.findMany({
    where: { status: "OPEN" },
    include: { category: { select: { name: true } }, quoteRequest: { select: { reference: true, title: true } } },
    orderBy: [{ eligibleSupplierCount: "asc" }, { lastDetectedAt: "desc" }],
    take: 20,
  });
