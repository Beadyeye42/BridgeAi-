...
export default async function AdminAffiliatesPage() {
  ...
  return <>
    ...
    <div className="dashboard-grid"><section className="panel panel-wide"><div className="panel-heading"><div><p className="eyebrow">Commercial oversight</p><h2>Affiliate subscribers and earnings</h2></div></div><div className="table-wrap"><table className="admin-table affiliate-admin-table"><thead><tr><th>Affiliate</th><th>Active paid</th><th>Qualifying / earning</th><th>Total referrals</th><th>This month</th><th>Lifetime net</th><th>Lost</th><th>Status</th></tr></thead><tbody>{snapshot.affiliates.map((affiliate) => {
      ...
      })}{!snapshot.affiliates.length && <tr><td colSpan={8}>No affiliates have been created yet. The first approved partner will hold one of ten limited places.</td></tr>}</tbody></table></div></section><aside className="panel"><p className="eyebrow">Administrator only</p><h2>Create affiliate</h2><p>Invitations are private. Activate only an approved partner you want inside the limited ten-place circle.</p><AffiliateCreateForm /></aside></div>
  </>;
}
