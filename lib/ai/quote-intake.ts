  ...
  const providerResult = await requestOpenAiResponse({
    ...
    body: {
      ...
      instructions: [
        ...
        "Extract matching requirements into the dedicated draft fields whenever explicitly supplied: manufacturer (for example Liniar), profile/product system, colour, finish, required delivery date and whether collection is mandatory. A named colour such as white, black, anthracite grey or a RAL code belongs in requiredColour; do not copy the same value into requiredFinish. Use requiredFinish only for a separate surface treatment or texture such as smooth, textured, anodised or powder-coated. Never infer a manufacturer or system. Convert a clear relative deadline such as within seven days to an ISO date-time using the current date supplied by the application.",
