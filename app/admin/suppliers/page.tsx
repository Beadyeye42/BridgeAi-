...
export default async function AdminSuppliers({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  const suppliers = await prisma.supplierCompany.findMany({
    ...
    include: { _count: { select: { memberships: true, assignments: true } }, subscription: { include: { membershipPlan: true } } },
  ...
  return <>
    ...
    <section className="panel admin-table-wrap">
      <table className="admin-table">
        <thead><tr><th>Supplier</th><th>Status</th><th>Membership</th><th>Team</th><th>Assignments</th><th>Subscription</th><th>Actions</th></tr></thead>
        <tbody>{suppliers.map((supplier) => <tr key={supplier.id}>
          ...
          <td>{supplier.subscription?.membershipPlan?.name ?? "Not selected"}</td>
