...
The customer-facing intake is universal: it asks what the customer needs and accepts a message, photo, drawing or document without exposing an industry selector. AI maps the evidence to the most specific active child category from the launched catalogue, and deterministic application logic prevents an unsupported category from being published. A request cannot reach confirmation until WHAT, WHERE, WHEN, quantity, price-critical specification and fulfilment method are present. PHE follow-ups depend on the inferred product: for example design heat loss/output for heat pumps, capacity and coil arrangement for cylinders, zones/floor build-up for underfloor heating, sizes/outputs for emitters, and duty data for pumps. A relevant schedule, schematic, calculation or drawing can satisfy that follow-up and prevents repeated questioning. Transport follows its own deterministic sequence without showing a form: item/photo and both route postcodes, access at each end, then carrying/loading help; date and price-critical load details are asked only if still missing. Completed steps are not repeated. Final equipment selection, sizing, compatibility, engineering, commissioning and regulatory compliance remain the supplier or installer's responsibility.

Future transport roadmap (not live): allow an approved operator to declare an empty return route and availability window, then match only suitable, correctly insured loads that fit the route, vehicle, capacity and membership rules. This must use explicit operator availability and safety controls; it must never be presented to buyers or suppliers as available until the matching, consent and operational controls are implemented and tested.
