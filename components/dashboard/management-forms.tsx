...
export function CoverageManager({ areas, collections, plan, companyBasePostcode }: { areas: CoverageView[]; collections: CollectionView[]; plan: CoveragePlanView; companyBasePostcode: string }) {
  ...
  const radiusOptions = useMemo(
    () => (plan?.tier === "HYPERLOCAL" ? [2] : [2, 5, 10, 15, 25, 40, 50, 75, 100]).filter((value) => purposeRadius === null || value <= (purposeRadius ?? 40)),
    [purposeRadius, plan?.tier],
  );
  ...
  const locationButton = <><button className="button button-outline" type="button" disabled={busy || locating} onClick={findMyLocation}>{locating ? <LoaderCircle className="spin" size={15}/> : <LocateFixed size={15}/>} {locating ? "Finding your postcode…" : "Use my current location"}</button><small className="body-copy">Your browser will ask for location permission. If it cannot provide your location, enter your postcode below. Bridge-iT does not save your exact coordinates.</small></>;
