...
import { AffiliateCreateForm, AffiliateProgrammeControl, AffiliateStatusControl } from "@/components/admin/affiliate-manager";
...
export default async function AdminAffiliatesPage() {
  ...
  return <>
    ...
    <section className="admin-affiliate-circle"><div><p className="eyebrow"><UsersRound size={14} /> Limited partner circle</p><h2>{snapshot.programme.maximumActive - activeCount} affiliate place{snapshot.programme.maximumActive - activeCount === 1 ? "" : "s"} remaining</h2><p>Bridge-iT will never activate more than {snapshot.programme.maximumActive} affiliates without changing the programme control deliberately.</p></div><strong>{activeCount}<span> / {snapshot.programme.maximumActive}</span></strong></section>
    ...
    <AffiliateProgrammeControl programme={{ maximumActive: snapshot.programme.maximumActive, commissionRateBps: snapshot.programme.commissionRateBps, qualificationPayments: snapshot.programme.qualificationPayments, commissionPayments: snapshot.programme.commissionPayments, validationDays: snapshot.programme.validationDays }} />
