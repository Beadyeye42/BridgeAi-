...
export async function getSupplierDashboard(supplierCompanyId: string) {
  const now = new Date();
  const [company, assignments, submittedCount, wonCount] = await Promise.all([
    prisma.supplierCompany.findUniqueOrThrow({
    ...
    prisma.supplierQuotation.count({
      where: { supplierCompanyId, status: { in: ["SUBMITTED", "ACCEPTED", "REJECTED"] } },
    }),
    prisma.supplierQuotation.count({ where: { supplierCompanyId, status: "ACCEPTED" } }),
  ]);

  return { company, assignments, submittedCount, wonCount, generatedAt: now };
}

export async function getSupplierRequest(supplierCompanyId: string, reference: string) {
  return prisma.supplierAssignment.findFirst({
    where: { supplierCompanyId, quoteRequest: { reference } },
    include: {
      quoteRequest: { include: { category: true, items: true, attachments: true } },
