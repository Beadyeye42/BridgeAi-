...
export function CoverageManager({ areas }: { areas: { id: string; type: string; label: string; postcodePrefix: string | null; centrePostcode: string | null; radiusMiles: number | null }[] }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await api("/api/supplier/coverage", "POST", body);
      setFeedback({ tone:"success", message:"Coverage saved. You are ready to receive matching enquiries." });
      event.currentTarget.reset(); setRadiusChoice("40"); setDepotPostcode(""); setPostcodePrefix(""); router.refresh();
...
export function TeamInviteForm() { const router=useRouter(); const [busy,setBusy]=useState(false); const [feedback,setFeedback]=useState<Feedback>(null); async function submit(event:React.FormEvent<HTMLFormElement>){event.preventDefault();setBusy(true);const data=new FormData(event.currentTarget);try{const result=await api("/api/supplier/team/invites","POST",{email:data.get("email"),role:data.get("role")});setFeedback({tone:"success",message:result.delivered?"Invitation email sent.":"Invitation created. Email delivery is not configured in this environment."});event.currentTarget.reset();router.refresh()}catch(e){setFeedback({tone:"error",message:e instanceof Error?e.message:"Invite failed"})}finally{setBusy(false)}} return <form className="panel form-section" onSubmit={submit}><div className="section-heading"><div><p className="eyebrow">Add a colleague</p><h2>Invite team member</h2></div><Users size={20}/></div><div className="form-stack"><Field label="Business email" name="email" type="email" value="" required/><label className="form-control"><span>Workspace role</span><select name="role"><option value="MEMBER">Member</option><option value="MANAGER">Manager</option></select></label><Result value={feedback}/><button className="button button-dark" disabled={busy}>{busy?<LoaderCircle className="spin" size={15}/>:<Send size={15}/>}Send invitation</button></div></form> }
