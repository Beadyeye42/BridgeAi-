...
export default async function AffiliateReferralsPage() {
  ...
  const { referrals, commissions, programme } = await prisma.$transaction(async (tx) => ({
    ...
    programme: await tx.affiliateProgramme.findUniqueOrThrow({ where: { id: "default" }, select: { qualificationPayments: true, commissionPayments: true } }),
  }));
  ...
  return <><div className="page-heading"><div><p className="eyebrow">Attribution</p><h1>My referrals</h1><p>Permanent supplier attribution and their live Stripe subscription progress.</p></div></div><section className="panel"><div className="table-wrap"><table className="data-table"><thead><tr><th>Supplier</th><th>Signup</th><th>Plan</th><th>Subscription</th><th>Referral status</th><th>Commission progress</th><th>Earnings</th><th>Next billing / cancellation</th></tr></thead><tbody>{referrals.map((referral) => {
    ...
    const progress = referral.successfulPaidPeriods > 0 && referral.successfulPaidPeriods <= programme.qualificationPayments
      ? `Qualification ${referral.successfulPaidPeriods} of ${programme.qualificationPayments}`
      : `${referral.eligibleCommissionPeriodsCompleted} of ${programme.commissionPayments}`;
    return <tr key={referral.referralId}><td>{referral.supplierName}</td><td>{(referral.signupAt ?? referral.referredAt).toLocaleDateString("en-GB")}</td><td>{referral.planName ?? referral.planCode ?? "—"}</td><td>{affiliateStatusLabel(referral.subscriptionStatus ?? "not started")}</td><td>{affiliateStatusLabel(referral.referralStatus)}</td><td>{progress}</td><td>{money(thisMonth)}<small>{money(total)} total</small></td><td>{referral.cancellationScheduledAt ? `Cancels ${referral.cancellationScheduledAt.toLocaleDateString("en-GB")}` : referral.currentPeriodEnd?.toLocaleDateString("en-GB") ?? "—"}</td></tr>;
