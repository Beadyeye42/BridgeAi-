...
type CoveragePlanView = { name: string; tier: string; maximumRadiusMiles: number | null; nationwideAllowed: boolean; maximumActiveOpportunities: number } | null;

export function CoverageManager({ areas, collections, plan }: { areas: CoverageView[]; collections: CollectionView[]; plan: CoveragePlanView }) {
  const router = useRouter();
  const [type, setType] = useState<"DISTANCE"|"NATIONWIDE">("DISTANCE");
  const [purpose, setPurpose] = useState<"SERVICE"|"DELIVERY">("DELIVERY");
  const radiusOptions = [10, 15, 25, 40, 50, 75, 100].filter((value) => plan?.maximumRadiusMiles === null || value <= (plan?.maximumRadiusMiles ?? 40));
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      form.reset(); setRadiusChoice(defaultRadius); setDepotPostcode(""); router.refresh();
  ...
  return <div className="management-form">
    <section className="panel form-section"><div className="section-heading"><div><p className="eyebrow">Your membership boundary</p><h2>{plan?.name ?? "Membership not configured"}</h2></div><Globe2 size={20}/></div><p className="body-copy">{plan?.nationwideAllowed ? "You may choose nationwide eligibility." : `You may choose any radius up to ${plan?.maximumRadiusMiles ?? 40} miles.`} Your chosen area can be smaller. Service and delivery rules never replace each other.</p><div className="detail-grid"><div><dt>Tier</dt><dd>{plan?.tier ?? "—"}</dd></div><div><dt>Active opportunities</dt><dd>Up to {plan?.maximumActiveOpportunities ?? 0}</dd></div></div></section>
    ...
    <form className="panel form-section" onSubmit={submit}><div className="section-heading"><div><p className="eyebrow">Add coverage</p><h2>Choose one clear rule</h2></div></div><label className="form-control"><span>What is this area for?</span><select value={purpose} onChange={(event)=>setPurpose(event.target.value as "SERVICE"|"DELIVERY")}><option value="SERVICE">Service or installation work</option><option value="DELIVERY">Product deliveries</option></select></label><div className="segmented"><button type="button" className={type === "DISTANCE"?"active":""} onClick={()=>setType("DISTANCE")}>Radius from company base</button>{plan?.nationwideAllowed && <button type="button" className={type === "NATIONWIDE"?"active":""} onClick={()=>setType("NATIONWIDE")}>Nationwide</button>}</div><div className="form-stack">{type === "DISTANCE" ? <>{locationButton}<label className="form-control"><span>Operating postcode</span><input name="centrePostcode" value={depotPostcode} onChange={(event)=>setDepotPostcode(event.target.value)} placeholder="For example GL52 6TD" required autoComplete="postal-code" /></label><label className="form-control"><span>Maximum distance</span><select value={radiusChoice} onChange={(event)=>setRadiusChoice(event.target.value)}>{radiusOptions.map((value)=><option value={value} key={value}>Within {value} miles</option>)}</select></label><input type="hidden" name="radiusMiles" value={radiusChoice}/><small className="body-copy">The complete area must remain inside your membership boundary from your registered company base.</small></> : <p className="body-copy">This makes you eligible across Great Britain, but exact product, capability, lead-time and capacity filters still apply.</p>}<Result value={feedback}/><SubmitButton busy={busy || locating} label="Save matching area" /></div></form></div>
