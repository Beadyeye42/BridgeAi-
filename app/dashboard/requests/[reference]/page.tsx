...
export default async function ConnectedRequestPage({ params }: { params: Promise<{ reference: string }> }) {
  ...
  const lifecycleStatus = request.status === "WON" ? "SELECTED" : request.status;
  const nextStep = supplierSelectionNextStep(request.category.slug, request.category.parent?.slug);
  ...
  return <PortalPage {...identity(session, company)} eyebrow={request.reference} title={request.title} description={request.category.name}>
    ...
    {isSelected && <section className={`won-job-banner lifecycle-${lifecycleStatus.toLowerCase()}`} role="status"><span>{lifecycleStatus === "COMPLETED" ? <CheckCircle2 size={24}/> : lifecycleStatus === "CANCELLED_AFTER_SELECTION" ? <XCircle size={24}/> : <Handshake size={24}/>}</span><div><p className="eyebrow">{lifecycleDisplay(lifecycleStatus)}</p><h2>{lifecycleStatus === "SELECTED" ? "You’ve been selected" : lifecycleDisplay(lifecycleStatus)}</h2><p>{lifecycleStatus === "SELECTED" ? "Good news—the customer has selected your quote to move forward." : lifecycleStatus === "CONFIRMED" ? "The order, booking or work has been agreed with the customer." : lifecycleStatus === "COMPLETED" ? "This job is recorded as completed." : "This selected opportunity did not become a confirmed job."}</p>{["SELECTED", "CONFIRMED"].includes(lifecycleStatus) && <p><b>Next step:</b> {lifecycleStatus === "SELECTED" ? nextStep : "Complete the agreed work, order, hire or booking, then record the outcome."}</p>}</div></section>}
    ...
    </div><aside className="request-action-rail"><section className="panel action-card"><div className="action-heading"><span><ShieldCheck size={18}/></span><div><p className="eyebrow">Your response</p><h2>Quotation</h2></div></div><ConnectedResponse assignmentId={assignment.id} status={assignment.status} quotationId={quotation?.id} quotationStatus={quotation?.status}/>{isSelected && <JobLifecycleControl reference={request.reference} status={lifecycleStatus}/>}</section><section className="panel request-facts"><h3>Request details</h3><Fact icon={<UserRound size={16}/>} label="Buyer" value={buyerTypeLabel(request.buyerType)}/><Fact icon={<MapPin size={16}/>} label="Delivery area" value={request.deliveryPostcode}/><Fact icon={<CalendarClock size={16}/>} label="Published" value={request.publishedAt?.toLocaleDateString("en-GB")??"—"}/><Fact icon={<Clock3 size={16}/>} label="Response due" value={request.responseDueAt.toLocaleDateString("en-GB")}/></section></aside></div>
  </PortalPage>;
}
