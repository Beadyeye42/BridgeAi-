...
export function MembershipSettings({ plans, matching, promotions }: { plans: Plan[]; matching: Matching; promotions: Promotion[] }) {
  ...
  return <div className="management-form">
    ...
    <div className="pricing-grid">{plans.map((plan)=><form className="panel form-section" key={plan.id} onSubmit={(event)=>savePlan(event, plan)}><div className="section-heading"><div><p className="eyebrow">{plan.tier}</p><h2>{plan.name}</h2></div><span className={`status-pill ${plan.active ? "approved" : "suspended"}`}>{plan.active ? "LIVE" : "OFFLINE"}</span></div><label className="form-control"><span>Public plan name</span><input name="name" defaultValue={plan.name} required/></label><label className="form-control"><span>Description</span><textarea name="description" defaultValue={plan.description ?? ""}/></label><label className="form-control"><span>Monthly price (£)</span><input name="monthlyPrice" readOnly={isFreeHyperlocalPlan(plan)} type="number" min={plan.monthlyPricePence === 0 ? "0" : "1"} step="0.01" defaultValue={(plan.monthlyPricePence/100).toFixed(2)} required/></label>{plan.tier !== "NATIONWIDE" && <label className="form-control"><span>Maximum radius (miles)</span><input name="maximumRadiusMiles" readOnly={isFreeHyperlocalPlan(plan)} type="number" min="1" max={plan.tier === "HYPERLOCAL" ? 10 : 500} defaultValue={plan.maximumRadiusMiles ?? ""} required/></label>}<label className="form-control"><span>Maximum live opportunities</span><input name="maximumActiveOpportunities" type="number" min="1" max="100" defaultValue={plan.maximumActiveOpportunities} required/></label><label className="form-control"><span>Display order</span><input name="displayOrder" type="number" min="0" max="1000" defaultValue={plan.displayOrder} required/></label><label className="form-control"><span>Stripe recurring Price ID</span><input name="providerPriceId" readOnly={isFreeHyperlocalPlan(plan)} defaultValue={plan.providerPriceId ?? ""} placeholder="Created securely if left blank"/></label><label className="toggle-row"><span><b>Plan available</b><small>Show this plan for eligible new and existing suppliers.</small></span><input name="active" type="checkbox" defaultChecked={plan.active}/></label><small className="body-copy">Stripe price: {isFreeHyperlocalPlan(plan) ? "not needed — free plan" : plan.providerPriceId ? "configured" : "created securely when first selected"}</small><button className="button button-dark" disabled={Boolean(busy)}>{busy===plan.id?<LoaderCircle className="spin" size={15}/>:<Save size={15}/>} Save plan</button></form>)}</div>
    <form className="panel form-section settings-width" onSubmit={saveMatching}>
      ...
      <p className="body-copy">Bridge-iT classifies every request after mandatory capability, subscription, capacity and location filters. Thin markets protect buyer fulfilment; dense markets use stronger exposure fairness among similarly qualified suppliers.</p>
