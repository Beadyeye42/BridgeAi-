...
export default async function BuyerRequestPage({ params }: { params: Promise<{ reference: string }> }) {
  ...
  return <><header className="buyer-page-head"><p className="eyebrow">{request.reference}</p><h1>{request.title}</h1><p>{request.summary}</p></header>
    <div className="buyer-two-column"><div>
      <section className="buyer-panel"><h2>Request details</h2><dl className="buyer-details"><div><dt>Status</dt><dd>{request.status.replaceAll("_", " ")}</dd></div><div><dt>Category</dt><dd>{request.category.name}</dd></div><div><dt>Location</dt><dd>{request.deliveryPostcode}</dd></div><div><dt>Required</dt><dd>{request.requiredBy?.toLocaleDateString("en-GB") ?? "Flexible"}</dd></div></dl>
        <h3>Items</h3>{request.items.map((item) => <div className="buyer-item" key={item.id}><b>{Number(item.quantity)} {item.unit} — {item.description}</b>{item.specification ? <p>{item.specification}</p> : null}</div>)}
      </section>
      <section className="buyer-panel"><h2>Files</h2>{request.attachments.map((file) => <a className="buyer-file" href={`/api/attachments/${file.id}/download`} key={file.id}>{file.fileName}<small>{file.mimeType} · {Math.ceil(file.byteSize / 1024)} KB</small></a>)}{!request.attachments.length ? <p>No files were attached.</p> : null}</section>
    </div><div>
      <section className="buyer-panel"><div className="buyer-section-head"><div><p className="eyebrow">Anonymous comparison</p><h2>Quotes</h2></div><span>{request.quotations.length}/5</span></div>
