...
export default async function AffiliateEarningsPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string; supplier?: string; plan?: string; status?: string }> }) {
  ...
  const { entries, summaries, plans, programme } = await prisma.$transaction(async (tx) => ({
    ...
    programme: await tx.affiliateProgramme.findUniqueOrThrow({ where: { id: "default" }, select: { commissionPayments: true } }),
  }));
