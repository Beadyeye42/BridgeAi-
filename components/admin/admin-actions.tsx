...
export function CategoryCreateForm({parents}:{parents:{id:string;name:string}[]}){const router=useRouter();const[busy,setBusy]=useState(false);const[message,setMessage]=useState("");async function submit(e:React.FormEvent<HTMLFormElement>){e.preventDefault();setBusy(true);const form=e.currentTarget;const d=new FormData(form);try{await call("/api/admin/categories","POST",{name:d.get("name"),slug:d.get("slug"),description:d.get("description"),active:true,parentId:d.get("parentId")||null});form.reset();setMessage("Category created.");router.refresh()}catch(err){setMessage(err instanceof Error?err.message:"Create failed")}finally{setBusy(false)}}return <form className="panel form-section" onSubmit={submit}><div className="section-heading"><div><p className="eyebrow">Catalogue</p><h2>Add category</h2></div><Plus size={20}/></div><div className="form-stack"><label className="form-control"><span>Product group</span><select name="parentId" defaultValue={parents[0]?.id??""}><option value="">Create a new top-level group</option>{parents.map(parent=><option value={parent.id} key={parent.id}>{parent.name}</option>)}</select></label><label className="form-control"><span>Name</span><input name="name" required/></label><label className="form-control"><span>Slug</span><input name="slug" pattern="[a-z0-9]+(?:-[a-z0-9]+)*" required/></label><label className="form-control"><span>Description</span><textarea name="description" rows={3}/></label><button className="button button-dark" disabled={busy}>{busy?<LoaderCircle className="spin" size={14}/>:<Plus size={14}/>}Create category</button>{message&&<p className="form-result">{message}</p>}</div></form>}
