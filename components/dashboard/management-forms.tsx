...
export function CompanyProfileForm({ company, categories, selectedCategoryIds }: { company: Record<string, unknown>; categories: { id: string; name: string; description: string | null }[]; selectedCategoryIds: string[] }) {
  ...
  return <form className="management-form" onSubmit={submit}>
    ...
    <section className="panel form-section" id="product-categories"><div className="section-heading"><div><p className="eyebrow">Matching</p><h2>Product categories</h2></div></div><div className="choice-grid">{categories.map((category) => <label className="choice-card" key={category.id}><input type="checkbox" name="categoryIds" value={category.id} defaultChecked={selectedCategoryIds.includes(category.id)} /><span><b>{category.name}</b><small>{category.description}</small></span></label>)}</div></section>
