...
- Supplier approval requires the legal company name, company address, Companies House number, director's name, company phone and company email. Insurance and accreditation documents are optional and do not block approval or quoting. The first 100 approved companies receive an immutable founding-supplier place. Automatic matching then requires an approved founding supplier, an active membership, the request product category and at least one active coverage rule. Suppliers can choose a postcode area, a 1–500 mile straight-line radius from one or more depot postcodes, or nationwide UK coverage. Distance uses postcode-centroid coordinates from Postcodes.io; it is an eligibility radius, not a driving-time promise. The assignment API repeats the match check transactionally before creating any assignment.
...
- Supplier billing uses Stripe Checkout: £29.99 per month for the first six billing months, then £49.99 per month. Ironbridge Group Ltd is not currently VAT registered, so VAT is not charged. Stripe Subscription Schedules enforce the price change. There are no introduction or winning fees. A customer selection transaction immediately creates the tenant-scoped contact-access grant, closes the request and audits both the selection and contact release.
...
Supplier winner emails use the same Resend account and are queued atomically when a customer selects a quotation. Active supplier team members receive the message unless they have disabled quotation-update emails. The queue uses locked claims, stable provider idempotency keys and five bounded retries; customer contact details are never put in email and remain behind the authenticated portal. `EMAIL_FROM` must use a sender on a domain verified in Resend.
...
Returning WhatsApp customers receive a simple two-choice command menu. `1` or `NEW QUOTE` starts an isolated intake session, while natural requests such as “another quote for aluminium bifolds” start the new session and retain the product details from that same message. `2` or `MY QUOTES` lists the customer’s five most recent requests without revealing supplier identities or contact details. `HELLO`, `MENU` and `HELP` show the menu without changing or deleting an unsent draft. Intake identifies the launched industry and exact product before asking only the relevant questions. PHE requests recognise boilers and packages, heat pumps, cylinders, underfloor heating, radiators, pipework, valves and controls, pumps and pressurisation, and mechanical plant packages. Schedules, schematics and heat-loss calculations are accepted alongside photos, drawings and PDFs; Bridge AI extracts procurement facts but does not perform engineering design or promise product suitability.
...
For billing, create two recurring monthly GBP Stripe Prices: £29.99 and £49.99. Set their IDs as `STRIPE_INTRODUCTORY_PRICE_ID` and `STRIPE_STANDARD_PRICE_ID`, then set `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET`. Register `https://<portal-host>/api/webhooks/stripe` for Checkout Session completion and customer subscription events. The verified webhook attaches a six-month introductory phase followed by the standard recurring phase. VAT collection is disabled while Ironbridge Group Ltd is not VAT registered; update the checkout configuration, customer-facing wording and Stripe Tax registration together if that changes. No success-fee product or checkout route is used.
