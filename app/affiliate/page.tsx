...
export default async function AffiliateDashboardPage({ searchParams }: { searchParams: Promise<{ months?: string }> }) {
  ...
  return <>
    <section className="affiliate-circle-hero">
      ...
      <div className="affiliate-circle-rate"><span>Your commission rate</span><strong>{commissionRate.toFixed(0)}%</strong><small>on eligible subscription revenue for {snapshot.programme.commissionPayments} paid commission months</small></div>
    </section>
    ...
    <div className="stats-grid">
      ...
      <article className="stat-card affiliate-stat"><div><p>Qualifying suppliers</p><b>{snapshot.qualifying}</b><span>{snapshot.programme.qualificationPayments} successful payment{snapshot.programme.qualificationPayments === 1 ? "" : "s"} required before commission</span></div></article>
