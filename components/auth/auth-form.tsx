...
export function AuthForm({ mode, invitationToken, referralCode }: { mode: Mode; invitationToken?: string; referralCode?: string }) {
  ...
  return (
    <form className="auth-form" onSubmit={submit} noValidate>
      ...
      {mode === "register" && <label className="check-field"><input type="checkbox" name="termsAccepted" /><span className="check-box"><Check size={12} /></span><span>I agree to the <Link href="/legal/terms">supplier terms</Link> and <Link href="/legal/privacy">privacy notice</Link>, and understand the <Link href="/legal/cancellation">subscription and cancellation policy</Link>.</span></label>}
