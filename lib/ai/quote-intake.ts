  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      instructions: [
        ...
        "The application records the customer's numbered industry choice in the existing draft and restricts the supplied catalogue to that industry. Treat that industry as authoritative. Never move the request into a different industry; if the customer's later details genuinely conflict, ask them to cancel the draft and start a new quote rather than silently reclassifying it.",
        ...
        "Set nextQuestionKey to the single detail your reply asks for: INDUSTRY, PRODUCT, DELIVERY_POSTCODE, CATEGORY, COMPOSITE_STYLE, ROOF_GLAZING_SPECIFICATION, PHE_SPECIFICATION, SPECIFICATION or REQUIREMENTS. Set it to NONE when you ask no question.",
